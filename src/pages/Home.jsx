...
function Home() {
  ...
  return (
    <main>
      ...
      <section
        id="header"
        className="container mx-auto max-w-7xl p-6 items-center md:space-x-24 !h-[calc(100vh-80px)] relative"
      >
        <figure className="w-2/5 hidden lg:block">
          <img src={HorrorMovie} className="translate-y-[-2rem] " alt="" />
        </figure>
        {/* Description */}
        <div className="flex items-center lg:w-3/5">
          <div className="flex flex-col mt-20 mb-48">
            <div className="bg-accent h-2 w-[80%] mb-4 hidden md:block"></div>
            <div className="font-medium text-5xl md:text-6xl leading-tight max-w-xl mb-12">
              Find your next favorite{" "}
              <span className="text-teal-300 font-bold">film.</span>
            </div>
            <form
              onSubmit={onSubmit}
              className="flex flex-col space-y-2 md:space-y-0 md:flex-row md:space-x-2 md:bg-darkest rounded"
            >
              <input
                type="text"
              ...
              />
              <button className="bg-accent flex items-center justify-center rounded text-white px-8 py-3 md:w-22 h-12">
                {!searchLoading ? (
      ...
      {/* Popular Movies */}
      <section id="popular" className="bg-darker relative">
        <div
          onClick={handleClickScroll}
          className="h-12 w-6 absolute border border-gray-300 rounded-full top-[-5rem] 
        left-1/2 translate-x-[-50%] p-[9px] hover:scale-105 active:scale-95 cursor-pointer transition"
        >
          <div className="w-1 h-1 bg-white absolute rounded-full scrollAnimation"></div>
        </div>
